...
@login_required
def index():
    ''' Home page of the application. It requires the user to be logged in'''
    form = PostForm()
...
def login():
    ...
    if loginform.validate_on_submit():
        ...
        ## Check if the user is set and password is valid
    ...
    return render_template('login.html', name="Sign In", form=loginform)
...
def register():
    ...
    if registrationForm.validate_on_submit():
        ...
        user.set_password(password=registrationForm.password.data)
        db.session.add(user)
        db.session.commit()
        flash('Congratulations, you are now a registered user')
        return redirect(url_for('login'))
    return render_template('register.html', name='Register', form=registrationForm)
